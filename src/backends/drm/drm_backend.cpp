...
void DrmBackend::handleUdevEvent()
{
    while (auto device = m_udevMonitor->getDevice()) {
        ...
        if (device->action() == QLatin1StringView("add")) {
            ...
            if (gpu) {
                if (primaryGpu() == gpu) {
                    qCCritical(KWIN_DRM, "Primary gpu %s has been removed!", qPrintable(gpu->drmDevice()->path()));
                    if (m_gpus.size() == 1) {
...
DrmGpu *DrmBackend::addGpu(const QString &fileName)
{
    ...
    connect(gpu, &DrmGpu::renderDeviceChanged, this, [this, gpu]() {
        if (primaryGpu() != gpu) {
            return;
        }
        qCCritical(KWIN_DRM, "Render device of primary GPU %s has changed", qPrintable(gpu->drmDevice()->path()));
        if (!gpu->renderDevice()) {
            // The udev event for the KMS node should be received later,
            // but the device is already unusable.
            gpu->setRemoved();
        }
        Q_EMIT aboutToChangeEglDisplay();
        sortGpus();
